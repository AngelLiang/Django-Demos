...
import {addCustomer, queryCustomer, removeCustomer, updateCustomer,queryCustomerVerboseName, queryCustomerListDisplay, queryCustomerDisplayOrder} from './service';
...
const TableList = () => {
  ...
  try {
    await updateCustomer(value, current_id);
    hide();
  ...
  useEffect(()=>{
    queryCustomerDisplayOrder().then(r => {
  ...
  useEffect(()=>{
    queryCustomerListDisplay().then(value => {
      ...
      <ProTable
        ...
        toolBarRender={(action, {selectedRows}) => [
          <Button type="primary" onClick={() => handleModalVisible(true)}>
            ...
            <Button type="primary" onClick={() => exportExcelAll(paramState, queryCustomer, table_columns, '客户-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{marginRight: 20}} placeholder="搜索客户 " onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows,table_columns, '客户-select')
                    }
                  }}
                  selectedKeys={[]}
                >
