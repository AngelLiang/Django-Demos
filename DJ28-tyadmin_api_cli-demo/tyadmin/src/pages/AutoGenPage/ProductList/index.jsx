...
import {addProduct, queryProduct, removeProduct, updateProduct,queryProductVerboseName, queryProductListDisplay, queryProductDisplayOrder} from './service';
...
const TableList = () => {
  ...
  try {
    await updateProduct(value, current_id);
    hide();
  ...
  useEffect(()=>{
    queryProductDisplayOrder().then(r => {
  ...
  useEffect(()=>{
    queryProductListDisplay().then(value => {
      ...
      <ProTable
        ...
        toolBarRender={(action, {selectedRows}) => [
          <Button type="primary" onClick={() => handleModalVisible(true)}>
            ...
            <Button type="primary" onClick={() => exportExcelAll(paramState, queryProduct, table_columns, '产品-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{marginRight: 20}} placeholder="搜索产品 " onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows,table_columns, '产品-select')
                    }
                  }}
                  selectedKeys={[]}
                >
