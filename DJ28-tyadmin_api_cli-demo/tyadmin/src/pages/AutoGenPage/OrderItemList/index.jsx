...
import {queryOrder, queryOrderVerboseName} from '@/pages/AutoGenPage/OrderList/service';import {queryProduct, queryProductVerboseName} from '@/pages/AutoGenPage/ProductList/service';
...
const TableList = () => {
                              ...
                              rules: [
                                {
                                  ...
                                  message: '产品为必填项',
                                    ...
                                    renderFormItem: (item, {value, onChange}) => {
                  return dealForeignKeyField(item, value, onChange, productForeignKeyList);
          },
              render: (text, record) => {
      return renderForeignKey(text, productVerboseNameMap);
    },
                            },{
  ...
  useEffect(() => {
    queryOrderVerboseName().then(value => {
      ...
      const [productForeignKeyList, setProductForeignKeyList] = useState([]);
      useEffect(() => {
        queryProduct({all: 1}).then(value => {
          setProductForeignKeyList(value);
        });
      }, []);
      const [productVerboseNameMap, setProductVerboseNameMap] = useState([]);
  useEffect(() => {
    queryProductVerboseName().then(value => {
      setProductVerboseNameMap(value);
    });
  }, []);
      ...
      <ProTable
        ...
        toolBarRender={(action, {selectedRows}) => [
          <Button type="primary" onClick={() => handleModalVisible(true)}>
            ...
            <Button type="primary" onClick={() => exportExcelAll(paramState, queryOrderItem, table_columns, '订单明细-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{marginRight: 20}} placeholder="搜索订单明细 " onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows,table_columns, '订单明细-select')
                    }
                  }}
                  selectedKeys={[]}
                >
