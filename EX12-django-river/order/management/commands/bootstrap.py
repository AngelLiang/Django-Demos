...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # 获取权限
        add_order_permission = Permission.objects.get(codename="add_order", content_type=content_type)
        change_order_permission = Permission.objects.get(codename="change_order", content_type=content_type)
        delete_order_permission = Permission.objects.get(codename="delete_order", content_type=content_type)
        ...
        team_leader_group.permissions.set([add_order_permission, change_order_permission,
                                           delete_order_permission, view_workflow_permission])
        # 采购员
        purchaser_group, _ = Group.objects.update_or_create(name="采购员")
        purchaser_group.permissions.set([change_order_permission, view_workflow_permission])

        # 创建状态
        draft_state, _ = State.objects.update_or_create(label="草稿", slug="draft")
        issued_state, _ = State.objects.update_or_create(label="已发货", slug="issued")
        partreceived_state, _ = State.objects.update_or_create(label="部分收货", slug="partreceived")
        received_state, _ = State.objects.update_or_create(label="收货完成", slug="received")
        closed_state, _ = State.objects.update_or_create(label="关闭", slug="closed")

        # 更新或创建工作流，并设置初始化状态
        workflow, _ = Workflow.objects.update_or_create(
            content_type=content_type, field_name="status", defaults={"initial_state": draft_state})

        draft_to_issued, _ = TransitionMeta.objects.update_or_create(
            workflow=workflow, source_state=draft_state, destination_state=issued_state)
        issued_to_partreceived, _ = TransitionMeta.objects.update_or_create(
            workflow=workflow, source_state=issued_state, destination_state=partreceived_state)
        issued_to_received, _ = TransitionMeta.objects.update_or_create(
            workflow=workflow, source_state=issued_state, destination_state=received_state)
        partreceived_to_received, _ = TransitionMeta.objects.update_or_create(
            workflow=workflow, source_state=partreceived_state, destination_state=received_state)
        received_to_issued, _ = TransitionMeta.objects.update_or_create(
            workflow=workflow, source_state=received_state, destination_state=issued_state)
        received_to_closed, _ = TransitionMeta.objects.update_or_create(
            workflow=workflow, source_state=received_state, destination_state=closed_state)

        # 草稿 -> 已发货
        draft_to_issued_meta, _ = TransitionApprovalMeta.objects.update_or_create(
            workflow=workflow, transition_meta=draft_to_issued)
        draft_to_issued_meta.groups.set([purchaser_group])

        # 已发货 -> 部分收货
        issued_to_partreceived_meta, _ = TransitionApprovalMeta.objects.update_or_create(
            workflow=workflow, transition_meta=issued_to_partreceived)
        issued_to_partreceived_meta.groups.set([purchaser_group])

        # 已发货 -> 收货完成
        issued_to_received_meta, _ = TransitionApprovalMeta.objects.update_or_create(
            workflow=workflow, transition_meta=issued_to_received)
        issued_to_received_meta.groups.set([purchaser_group])

        # 收货完成 -> 已发货
        received_to_issued_meta, _ = TransitionApprovalMeta.objects.update_or_create(
            workflow=workflow, transition_meta=received_to_issued)
        received_to_issued_meta.groups.set([team_leader_group])

        # 收货完成 -> 关闭
        received_to_closed_meta, _ = TransitionApprovalMeta.objects.update_or_create(
