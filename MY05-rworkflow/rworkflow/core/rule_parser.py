"""
>>> test_eq_rule1 = ['=', 1, 1]
>>> rp = RuleParser(test_eq_rule1)
>>> rp.evaluate()
True

>>> test_eq_rule2 = ['=', 1, 2]
>>> rp = RuleParser(test_eq_rule2)
>>> rp.evaluate()
False

>>> test_neq_rule1 = ['!=', 1, 1]
>>> rp = RuleParser(test_neq_rule1)
>>> rp.evaluate()
False

>>> test_neq_rule2 = ['!=', 1, 2]
>>> rp = RuleParser(test_neq_rule2)
>>> rp.evaluate()
True

>>> test_gt_rule1 = ['>', 2, 1]
>>> rp = RuleParser(test_gt_rule1)
>>> rp.evaluate()
True

>>> test_plus_rule1 = ['=', ['+', 1, 2], 3]
>>> rp = RuleParser(test_plus_rule1)
>>> rp.evaluate()
True

>>> test_divide_rule1 = ['=', ['/', 10.5, 2], 5.25]
>>> rp = RuleParser(test_divide_rule1)
>>> rp.evaluate()
True
"""
    ...
    def evaluate(self):
        ...
        if not isinstance(ret, bool):
            logger.warn('In common usage, a rule must return a bool value,'
                        'but get {}, please check the rule to ensure it is true', ret)
        return ret
